...
class CropsFromTrialsIterator(object):
    ...
    def get_batches(self, dataset, shuffle):
        ...
        i_trial_stops = [trial.shape[0] for trial in dataset.X]
...
def _create_batch_from_i_trial_start_stop_blocks(X, y, i_trial_start_stop_block, n_preds_per_input=None):
    Xs = []
    ys = []
    for i_trial, start, stop in i_trial_start_stop_block:
        Xs.append(np.transpose(X[i_trial][start:stop, :]))
