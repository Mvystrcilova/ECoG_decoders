...
if __name__ == '__main__':
    ...
    data = Data(home + '/previous_work/ALL_11_FR1_day1_absVel.mat', num_of_folds=5)
    ...
    if data.num_of_folds == -1:
        writer = get_writer(f'/logs/model_1_lr_0.001/cv_run_{1}')
        ...
        monitors = [('correlation monitor', correlation_monitor), ('checkpoint', Checkpoint(monitor=monitor,
                                                                                            f_params=home + "/logs/model_1_lr_0.001/params/params_{last_epoch[epoch]}.pt",
                                                                                            f_history=home + '/logs/model_1_lr_0.001/histories/history_{last_epoch[epoch]}.json',
                                                                                            f_optimizer=home + '/logs/model_1_lr_0.001/optimizers/optimizer_{last_epoch[epoch]}.pt',
                                                                                            )),
        ...
        torch.save(model.model, home + '/models/saved_models/initial_model')
        regressor.max_correlation = -1000

        regressor.fit(data.train_set.X, data.train_set.y)
        # regressor.fit(data.train_set.X, data.train_set.y)

        torch.save(model.model, home + '/models/saved_models/experiment_1')
